...
def VPGImage_5(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_params, vf_type="zero", animate=False, use_adaptive_stepsize=False, split_level=0, sup_learning_steps = 10):
    ...
    w1, b1 = define_vars(2, 32, "h1", weight_init=normc_initializer(1.0))
    sy_h1 = lrelu(dense(sy_ob_no, w1, b1)) # hidden layer

    w2, b2 = define_vars(32, 32, "h2", weight_init=normc_initializer(1.0))
    sy_h2 = lrelu(dense(sy_h1, w2, b2)) # hidden layer

    w3, b3 = define_vars(32, 32, "h3", weight_init=normc_initializer(1.0))
    sy_h3 = lrelu(dense(sy_h2, w3, b3)) # hidden layer

     #---------------------Last 2 FC layers--------------------------#   
   
    w4, b4 = define_vars(32, 32, "h4", weight_init=normc_initializer(1.0))
    sy_h4 = lrelu(dense(sy_h3, w4, b4)) # hidden layer
...
if __name__ == "__main__":
    ...
    VPGImage_5(logdir=None, seed=0, desired_kl=2e-3, vf_type='linear', vf_params={}, use_adaptive_stepsize=False, split_level=0, sup_learning_steps = 1, **general_params)
