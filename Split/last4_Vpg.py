...
def cartpoleSplit_4(seed = 0, numBatches=50, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True, logdir=None, vf_type='linear'):
    ...
    with tf.variable_scope("SL_vars"):
        critical_layer = dense(sy_ob_no, 16, "criticalLayer", weight_init=normc_initializer(1.0)) # hidden layer
    
    with tf.variable_scope("PG_vars"):
        #The weights of the two following layers are in the PG scope
        
        sy_h1 = lrelu(dense(critical_layer, 32, "h1", weight_init=normc_initializer(1.0))) # hidden layer
    ...
    with tf.variable_scope("PG_vars", reuse = True):
        ...
        h1_back = lrelu(dense(activation, 32, "h1", weight_init=normc_initializer(1.0))) # hidden layer
        h2_back = lrelu(dense(h1_back, 32, "h2", weight_init=normc_initializer(1.0))) # hidden layer
    ...
    for i in range(numBatches):
        ...
        while True:
            ...
            while True:
                ...
                if timesteps_this_batch>=numTimestepsBatch:
                    flag = False
                    break     
                if done:
                    break                    
       ...
       # print(sess.run(activation))
        ...
        SL_Loss.append(slLoss)
        PG_Loss.append(pgLoss)
