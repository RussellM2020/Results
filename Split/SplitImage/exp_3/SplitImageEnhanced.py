...
clear("VPGImage_5Conv_reward.dat")
clear("VPGImage_5Conv_pg.dat")
...
def SplitImage(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_params, vf_type="zero", animate=False, use_adaptive_stepsize=False, split_level=0, sup_learning_steps = 10):
...
# ?, 42,42,3    ---> 5,3,10 ---> ?, 42 ,42, 10 ----->5,10,10 ---> ?,42,42,10
#conv_vars(filter_size, num_channels(input depth), num_filters(output depth))



    w11, b11 = define_conv_vars(filter_size[0], 3, num_filters[0], "h11")
    sy_h11 = conv2d(sy_ob_no, w11, b11) # hidden layer

    w12, b12 = define_conv_vars(filter_size[1], num_filters[0], num_filters[1], "h12")
    sy_h12 = conv2d(sy_h11, w12, b12) # hidden layer

    #Flattening
    shape_sy_h12 = tf.shape(sy_h12)
    flat_shape = tf.reduce_prod(shape_sy_h12[1:])
    sy_h12_flat = tf.reshape(sy_h12, [-1, flat_shape])



    w13, b13 = define_vars(17640, 432, "h13", weight_init=normc_initializer(1.0))
    sy_h13_flat = dense(sy_h12_flat, w13, b13)

    sy_h13 = tf.reshape(sy_h13_flat, [-1, 12, 12,3])
    ...
    w21, b21 = define_conv_vars(filter_size[0], 3, num_filters[0], "h21")
    sy_h21 = conv2d(sy_h13, w21, b21) # hidden layer

    w22, b22 = define_conv_vars(filter_size[1], num_filters[0], num_filters[1], "h22")

    sy_h22 = conv2d(sy_h21, w22, b22) # hidden layer

    #Flattening
    shape_sy_h22 = tf.shape(sy_h22)
    flat_shape = tf.reduce_prod(shape_sy_h22[1:])
    sy_h22_flat = tf.reshape(sy_h22, [-1, flat_shape])



    w23, b23 = define_vars(1440, 192, "h23", weight_init=normc_initializer(1.0))
    sy_h23_flat = dense(sy_h22_flat, w23, b23)

    sy_h23 = tf.reshape(sy_h23_flat, [-1, 8, 8, 3])

    #------------------End of Conv Block 2------------------------#

    #---------------------Conv Block 3-----------------------#

    w31, b31 = define_conv_vars(filter_size[0], 3, num_filters[0], "h31")
    sy_h31 = conv2d(sy_h23, w31, b31) # hidden layer

    w32, b32 = define_conv_vars(filter_size[1], num_filters[0], num_filters[1], "h32")
    sy_h32 = conv2d(sy_h31, w32, b32) # hidden layer

    #Flattening
    shape_sy_h32 = tf.shape(sy_h32)
    flat_shape = tf.reduce_prod(shape_sy_h32[1:])
    sy_h32_flat = tf.reshape(sy_h32, [-1, flat_shape])



    w33, b33 = define_vars(640, 32, "critical_layer", weight_init=normc_initializer(1.0))
    critical_layer = dense(sy_h32_flat, w33, b33)
    ...
    h4_back = lrelu(dense(act_var1, w4, b4)) 
        
    sy_mean_back = dense(h4_back, wmean, bmean)
    ...
    sy_logprob_n = -tf.reduce_sum(logstd) - ac_dim*0.5*tf.log(2*np.pi) - 0.5*tf.reduce_sum(tf.square(((sy_mean_back - sy_ac_n)/tf.exp(logstd))), reduction_indices=-1)
    ...
    _SLvars = [w11, b11, w12, b12, w13, b13, w21, b21, w22, b22, w23, b23, w31, b31, w32, b32, w33, b33]
    _PGvars = [ act_var1, w4, b4, wmean, bmean]
    ...
    optimizerPG = tf.train.AdamOptimizer(1e-3)
    PG_step = optimizerPG.minimize(loss = sy_surr, var_list = _PGvars)
       
    optimizerSL = tf.train.AdamOptimizer(1e-5)
    ...
    for i in range(n_iter):
        ...
        while True:
            ...
            while True:
                ...
                ac = sess.run(sy_sampled_ac, feed_dict={sy_ob_no : [ob]})[0] #TODO: Change this accordingly
                acs.append(ac)
            ...
            path = {"observation" : np.array(obs), "terminated" : terminated,
                    "reward" : np.array(rewards), "action" : np.array(acs), "dist": np.array(dists)}
            paths.append(path)
        ...
        sess.run(SL_step, feed_dict={sy_ob_no:ob_no} )
        pgloss = sess.run(sy_surr,feed_dict={sy_ob_no:ob_no, sy_ac_n:ac_n, sy_adv_n:standardized_adv_n} )
        slloss = sess.run(l2_loss, feed_dict = {sy_ob_no:ob_no})
        ...
        # Log diagnostics
        meanDist = np.mean([path["dist"].sum() for path in paths])

        logz.log_tabular("EpMeanDist", meanDist)
        meanReward = np.mean([path["reward"].sum() for path in paths])
        logz.log_tabular("EpRewMean", meanReward)
        logz.log_tabular("EpLenMean", np.mean([pathlength(path) for path in paths]))
        #logz.log_tabular("KLOldNew", kl)
       # logz.log_tabular("Entropy", ent)
        #logz.log_tabular("SL Loss", sl_loss)
        logz.log_tabular("PG Loss", pgloss)
        logz.log_tabular("SL Loss", slloss)
        RewardHistory.append(meanReward)
        PGHistory.append(pgloss)
        SLHistory.append(slloss)
        ...
        if (i%100==0) and (i!=0):
            ...
            store("SplitImage_DistHistory.dat", DistHistory)
            store("SplitImage_RewardHistory.dat", RewardHistory)
            store("SplitImage_PGHistory.dat", PGHistory)
            store("SplitImage_SLHistory.dat", SLHistory)
