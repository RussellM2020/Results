...
def load_weights(sess, filename):
    ...
    for v in vars:
        #if ("h11" in v.name) or ("h12" in v.name):
...
def Pose_predict(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_params, vf_type="zero", animate=False, use_adaptive_stepsize=False, split_level=0, sup_learning_steps = 10):
    tf.set_random_seed(seed)
    np.random.seed(seed)

    env = normalize(PointImageEnv())
    #ob_dim = env.observation_space.flat_dim
    ...
    sy_target = tf.placeholder(shape = [None,2], name = "target", dtype = tf.float32)
    ...
    w13, b13 = define_vars(17640, 32, "h13", weight_init=normc_initializer(1.0))
    sy_h13 = lrelu(dense(sy_h12_flat, w13, b13))
     #---------------------Last 2 FC layers--------------------------#   
   
    

    wmean, bmean = define_vars(32, 2, "pos", weight_init=normc_initializer(0.1))
    sy_out = dense(sy_h13, wmean, bmean) # hidden layer
    ...
    l2_loss = tf.nn.l2_loss(tf.subtract(sy_out, sy_target))
        #activationOp will change during the PG_step
    _SLvars = [w11, b11, w12, b12, w13, b13, wmean, bmean]
    # _PGvars = [ w11, b11, w12, b12, w13, b13, w21, b21, wmean, bmean]
    ...
    for i in range(n_iter):
        ...
        state_no =  np.concatenate([path["state"] for path in paths])
        ...
        # print(sess.run(act_var1))
        sess.run(SL_step, feed_dict={sy_ob_no:ob_no, sy_target: state_no} )
        #pgloss = sess.run(sy_surr, feed_dict={sy_ob_no:ob_no, sy_ac_n:ac_n, sy_adv_n:standardized_adv_n})
        slloss = sess.run(l2_loss, feed_dict = {sy_ob_no:ob_no, sy_target:state_no})
       ...
       # logz.log_tabular("Entropy", ent)
        ...
        if i%100==0:
            #store("VPGImage_5Conv_dist.dat", )
            store("PosePredictor_Loss.dat", SLHistory)
...
if __name__ == "__main__":
    general_params = dict(gamma=0.97, animate=False, min_timesteps_per_batch=4000, n_iter=5000, initial_stepsize=1e-3)
    Pose_predict(logdir=None, seed=0, desired_kl=2e-3, vf_type='linear', vf_params={}, use_adaptive_stepsize=False, split_level=0, sup_learning_steps = 1, **general_params)
