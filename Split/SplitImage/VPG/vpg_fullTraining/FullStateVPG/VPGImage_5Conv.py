...
import logz
import scipy.signal
from image_env import PointImageEnv
...
def VPGImage_5Conv(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_params, vf_type="zero", animate=False, use_adaptive_stepsize=False, split_level=0, sup_learning_steps = 10):
    ...
    w13, b13 = define_vars(17640, 32, "critical_layer", weight_init=normc_initializer(1.0))
    ...
    for i in range(n_iter):
       ...
       # logz.log_tabular("Entropy", ent)
        ...
        DistHistory.append(meanDist)

        if i%100==0:
            #store("VPGImage_5Conv_dist.dat", )
            store("VPGImage_5Conv_Modreward.dat", RewardHistory)
            store("VPGImage_5Conv_pg.dat", PGHistory)
            RewardHistory=[]
            DistHistory  = []
            PGHistory=[]
...
if __name__ == "__main__":
    ...
    VPGImage_5Conv(logdir=None, seed=0, desired_kl=2e-3, vf_type='linear', vf_params={}, use_adaptive_stepsize=False, split_level=0, sup_learning_steps = 1, **general_params)
