...
def SplitImage(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_params, vf_type="zero", animate=False, use_adaptive_stepsize=False, split_level=0, sup_learning_steps = 10):
    ...
    w13, b13 = define_vars(4000, 32, "critical_layer", weight_init=normc_initializer(1.0))
    critical_layer = dense(sy_h12_flat, w13, b13)
    ...
    w21, b21 = define_vars(32, 32, "h21", weight_init=normc_initializer(1.0))
    sy_h21 = lrelu(dense(critical_layer, w21, b21)) # hidden layer
    ...
    sy_mean = dense(sy_h21, wmean, bmean) # hidden layer

    
    #------------------------------------------------------------------#


    act_var1 = tf.Variable(initial_value = tf.zeros([ min_timesteps_per_batch+1,32], dtype = tf.float32),name = "act_var1", trainable = True)
    ...
    _PGvars = [ act_var1, w21, b21, wmean, bmean]
    ...
    optimizerPG = tf.train.AdamOptimizer(1e-5)
