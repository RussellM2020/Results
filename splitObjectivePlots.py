...
def main_cartpole(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True, logdir=None, vf_type='linear'):
    ...
    with tf.variable_scope("PG_vars"):
        ...
        activationOp =  tf.Variable(initial_value = tf.zeros([10,16], tf.float32), name = "activation", validate_shape = False, trainable=True)
        activationOp = tf.assign( activationOp, critical_layer, False)
    ...
    for i in range(num_trajectories):
        ...
        print("Activation value before PG step")
        print(value)
        sess.run(PG_step, feed_dict={sy_ob_no:ob_no, sy_ac_n:ac_n, sy_adv_n:standardized_adv_n} )
        print("Activation value after PG step")
        print(sess.run(activationOp))
        ...
        PGloss_History.append(PGloss)
        SLloss_History.append(SLloss)
        logz.log_tabular("EpRewMean", EpRewMean)
        logz.log_tabular("PGloss", PGloss)
        logz.log_tabular("SLloss", SLloss)
    ...
    plt.plot(Iterations, PGloss_History, '-r', label="PGloss")
    plt.plot(Iterations, SLloss_History, '-b', label="SLloss")

    plt.plot(Iterations, MeanRewardHistory, '-g', label="Rewards")
