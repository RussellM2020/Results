...
def main_cartpole(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True, logdir=None, vf_type='linear'):
    ...
    _PGvarsInitial = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='PG_vars')    
    _SLvarsInitial = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='SL_vars')
    _varsInitial = _PGvarsInitial + _SLvarsInitial

    sess = tf.InteractiveSession(config = tf_config)
    op = tf.variables_initializer(var_list = _varsInitial )
    sess.run(op)
    #print(_PGvarsInitial)
    #for variable in _PGvarsInitial:
        #print(variable,)
        #print(sess.run(tf.is_variable_initialized(variable)))
   
   # tf.global_variables_initializer().run() #pylint: disable=E1101
   #variables for the first smapling action are initialized 
    if vf_type == 'linear':
    ...
    for i in range(n_iter):
        ...
        with tf.variable_scope("PG_vars", reuse = True):
            ...
            activationVariable = tf.assign(variableInit, actValue, False)
        ...
        if (i!=0):
            indicator = True        
        with tf.variable_scope("PG_optimizer", reuse= indicator):

            optimizerPG = tf.train.AdamOptimizer()
            PG_step = optimizerPG.minimize(loss = sy_surr, var_list = _PGvars)

        with tf.variable_scope("SL_optimizer", reuse= indicator):
            optimizerSL = tf.train.AdamOptimizer()

            SL_step = optimizerSL.minimize(loss = cross_entropy, var_list = _SLvars)
