...
def main_cartpole(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True, logdir=None, vf_type='linear'):
    ...
    with tf.variable_scope("PG_vars"):
        ...
        variableActivation = tf.Variable(initial_value = tf.ones(10,16), trainable = True)
    ...
    sy_logits_feedforward = dense(sy_h3, num_actions, "final", weight_init=normc_initializer(0.05))
    ...
    sy_sampled_ac = categorical_sample_logits(sy_logits_feedforward)[0] # sampled actions, used for defining the policy (NOT computing the policy gradient)
    ...
    for i in range(n_iter):
        ...
        ActivationVals = sess.run(critical_layer, feed_dict={sy_ob_no: ob_no})
        with tf.variable_scope("PG_vars"):
            variableActivation = tf.variable(initial_value = np.zeros(10), trainable=True)        
            tf.assign(variableActivation,ActivationVals )
           
            back_h3 = lrelu(dense(variableActivation, 32, "h3", weight_init=normc_initializer(1.0))) # hidden layer 
                                                                                                                               
               
            sy_logits_back = dense(back_h3, num_actions, "final", weight_init=normc_initializer(0.05))
